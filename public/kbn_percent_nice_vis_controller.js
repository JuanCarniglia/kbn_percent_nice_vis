    ...
    function ($scope, $element, $rootScope, Private) {
        ...
        var _buildVis = function _buildVis(root) {
            ...
            if (results.length == 2) {
                ...
                var svgResult = d3.select(svgRoot).append('svg').attr("role", "values_container").attr("class", "svgNumberContainer").append('g');

                svgResult.append("svg:text").attr("class", "svgNumberLabel").attr("x", "0px").attr("y", "2px").text(results[0].label);

                var valueFormatted = results[0].formatNumber ? numeral(value).format(results[0].formatNumber) : value;

                svgResult.append("svg:text").attr("class", "svgNumberValue").attr("x", "0px").attr("y", "30px").style("fill", results[0].valueColor).text(valueFormatted);
        ...
        var _render = function _render(data) {
            ...
            if (!retVal){
                ...
                svgResult.append("svg:text").attr("class", "svgNumberLabel").attr("x", "0px").attr("y", "2px").text("Division by Zero!");
