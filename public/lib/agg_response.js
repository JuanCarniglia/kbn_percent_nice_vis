...
define(function (require) {
    return function percentNiceProvider(Private, Notifier) {
        ...
        return function (vis, resp) {
            ...
            // some validations
            if (!vis.aggs.bySchemaGroup.metrics[0]) return null;

            if (!vis.aggs.bySchemaGroup.buckets[0]) return null;
            ...
            _.each(vis.aggs, function (d, i) {
                ...
                if (labels.length > pos) {
                    ...
                    nodes.push({ 'type': type, 'value': value, 'label': labels[pos].text, 'formatNumber': labels[pos].numeralFormat ? labels[pos].numeralFormat : null, valueColor: valueColor, 'image': image });
                    
                } else {
                    nodes.push({ 'type': type, 'value': value, 'label': type, 'image': image });
                }
